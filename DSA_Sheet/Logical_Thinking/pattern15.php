...
<head>
  ...
  <title>Pattern-15: Reverse Letter Triangle Pattern</title>
</head>
  <style>
        svg{
            cursor: pointer;
...
<body class="bg-light py-4">
  ...
  <div class="container-fluid">
    <div class="row">
      ...
      <div class="col-8">
        <div>
          <h2><strong>Pattern-15: Reverse Letter Triangle Pattern</strong></h2>
          ...
          <p class="mb-3"><strong>Problem Statement:</strong> Given an integer <strong>N</strong>, print the following
            pattern.</p>
          <img src="https://static.takeuforward.org/wp/uploads/2023/02/image-19.png" alt class="wp-image-7549"
...
A
        ...
        <div>
          <p>This pattern problem is very similar to the last pattern problem we did where we had to print an increasing
            letter pyramid pattern but this time we have to print it in the reverse fashion. So, the outer loop will run
            for N rows and the inner loop will loop for N-i-1 alphabets in each row (where i is the row number) because
            the 1st row will have alphabets from A to A+(N-1). Alphabets in each row will start from A each time we
            enter a new row and will loop till (A+N-i-1)th alphabet in that row. In this way, the last row will only
            contain the alphabet A at last.</p>
        </div>
        <!-- yaha code karna hai  -->
        <div class="container my-5">
          <div class="card shadow">
            <h3 class="mb-4 text-start">Inverted Alphabet Triangle Pattern</h3>
            <div class="text-center mb-3 card-header d-flex justify-content-between align-items-center">
              ...
              <div>
                <button id="CopyCppBtn" onclick="copyCode('CodeCpp15')"
                  class="btn btn-sm btn-primary copy-btn">Copy</button>
                <button id="CopyJavaBtn" onclick="copyCode('CodeJava15')"
            ...
            <div id="CodeCpp" class="code-content">
              <div id="CodeCpp15" class="code-box">
                <div><span class="keyword">#include</span> &lt;bits/stdc++.h&gt;</div>
                <div><span class="keyword">using namespace</span> std;</div>
                <br>
                <div><span class="keyword">void</span> <span class="function">pattern15</span>(<span
                ...
                <div> <span class="comment">// Print characters from 'A' to 'A' + (N-i-1)</span></div>
                <div> <span class="keyword">for</span> (<span class="keyword">char</span> ch = 'A'; ch &lt;= 'A' + (N -
                  i - 1); ch++) {</div>
                <div> cout &lt;&lt; ch &lt;&lt; " ";</div>
                <div> }</div>
                <div> cout &lt;&lt; endl;</div>
                ...
                <div> <span class="comment">// Value of N</span></div>
            ...
            <div id="codeJava" class="code-content d-none">
              <div id="CodeJava15" class="code-box">
                <div><span class="keyword">public class</span> <span class="class-name">Main</span> {</div>
                <br>
                <div> <span class="comment">// Method to print the inverted alphabet triangle pattern</span></div>
                <div> <span class="keyword">static void</span> <span class="function">pattern15</span>(<span
                    class="keyword">int</span> N) {</div>
                <div> <span class="comment">// Outer loop for rows</span></div>
                <div> <span class="keyword">for</span> (<span class="keyword">int</span> i = 0; i &lt; N; i++) {</div>
                <div> <span class="comment">// Print characters from 'A' to 'A' + (N-i-1)</span></div>
