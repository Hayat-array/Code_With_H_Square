...
<head>
  ...
  <title>Pattern - 21: Hollow Rectangle Pattern</title>
</head>
  <style>
        svg{
            cursor: pointer;
...
<body class="bg-light py-4">
  ...
  <div class="container-fluid">
    <div class="row">
      ...
      <div class="col-8">
        <div>
          <h2><strong>Pattern - 21: Hollow Rectangle Pattern</strong></h2>
          ...
          <p class="mb-3"><strong>Problem Statement:</strong> Given an integer <strong>N</strong>, print the following
            pattern.</p>
          <img src="https://static.takeuforward.org/wp/uploads/2023/02/image-25.png" alt class="wp-image-7549"
...
******
        ...
        <div>
          <p>This problem is a very simple one where we just have to print a hollow rectangle. So, after observation, we
            see that the stars are only printed at the border of the rectangle, and on the rest of the positions, spaces
            are printed. Hence, the outer loop and inner loop both will loop for n times and in the inner loop we add a
            condition that if the row index or column index is equal to 0 or n-1, then stars should get printed
            otherwise spaces should be printed.</p>
        </div>
        <!-- yaha code karna hai  -->
        <div class="container my-5">
          ...
          <div class="card shadow">
            <!-- Title -->
            <h3 class="mb-4 text-start">Hollow Square Star Pattern</h3>
            ...
            <div class="text-center mb-3 card-header d-flex justify-content-between align-items-center">
              ...
              <div>
                <button id="CopyCppBtn" onclick="copyCode('CodeCpp21')"
                  class="btn btn-sm btn-primary copy-btn">Copy</button>
                <button id="CopyJavaBtn" onclick="copyCode('CodeJava21')"
            ...
            <div id="CodeCpp" class="code-content">
              <div id="CodeCpp21" class="code-box">
                <div><span class="keyword">#include</span> &lt;bits/stdc++.h&gt; <span class="comment">// Includes all
                    standard C++ libraries</span></div>
                <div><span class="keyword">using namespace</span> std; <span class="comment">// Avoids using std::
                    prefix</span></div>
                <br>
                <div><span class="keyword">void</span> <span class="function">hollowSquarePattern</span>(<span
                    class="keyword">int</span> n) {</div>
                <div> <span class="comment">// Outer loop runs 'n' times for each row</span></div>
                <div> <span class="keyword">for</span> (<span class="keyword">int</span> i = 0; i &lt; n; i++) {</div>
                <div> <span class="comment">// Inner loop runs 'n' times for each column</span></div>
                <div> <span class="keyword">for</span> (<span class="keyword">int</span> j = 0; j &lt; n; j++) {</div>
                <div> <span class="comment">// Print '*' at the borders, otherwise print space</span></div>
                <div> <span class="keyword">if</span> (i == 0 || j == 0 || i == n-1 || j == n-1)</div>
                <div> cout &lt;&lt; "*";</div>
                <div> <span class="keyword">else</span></div>
                <div> cout &lt;&lt; " ";</div>
                <div> }</div>
                <div> <span class="comment">// Move to the next line after each row</span></div>
                <div> cout &lt;&lt; endl;</div>
                <div> }</div>
                <div>}</div>
                <br>
                ...
                <div> hollowSquarePattern(n);</div>
                <div> <span class="comment">// Return 0 to indicate successful execution</span></div>
            ...
            <div id="CodeJava" class="code-content d-none">
              <div id="CodeJava21" class="code-box">
                <div><span class="keyword">public class</span> <span class="class-name">Main</span> {</div>
                <div> <span class="comment">// Function to print the hollow square pattern</span></div>
                <div> <span class="keyword">static void</span> <span class="function">hollowSquarePattern</span>(<span
                    class="keyword">int</span> n) {</div>
                <div> <span class="comment">// Outer loop runs 'n' times for each row</span></div>
                <div> <span class="keyword">for</span> (<span class="keyword">int</span> i = 0; i &lt; n; i++) {</div>
                <div> <span class="comment">// Inner loop runs 'n' times for each column</span></div>
                ...
                <div> }</div>
                <br>
                <div> <span class="comment">// Main method - entry point of the program</span></div>
